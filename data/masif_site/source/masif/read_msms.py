...
def read_msms(file_root):
    ...
    # EECS545 note:
    # modified lines 14-17, had type issues with my version of msms
    vertfile = open(file_root + ".vert", 'rb')
    data = vertfile.read().rstrip()
    meshdata = str(data).split("\\n")
    vertfile.close()
    ...
    # Read faces.
    facefile = open(file_root + ".face", 'rb')
    meshdata = str(facefile.read().rstrip()).split("\\n")
    facefile.close()
